...
/*
* Then find NAPFU24 date
*/
foreach ($results as $result) {
 $candid = $result['CandID'];
 $pscid = $result['PSCID'];
 $query = "select date_visit from session where CandID=$candid and visit_label='NAPFU24'";
 $NAPFU24Date["$pscid"] = $DB->pselectOne($query, array());
 if (empty($NAPFU24Date["$pscid"])) {
  echo "Cannot get NAPFU24Date for $pscid\n";
//  die();
 }
}

$count = 0;
foreach ($NAPFU24Date as $key => $value) {
 if ($count == 0) {
  $w = " AND ((commentid like '%$key%' AND date_taken<='$value')";
 } else {
  $w = $w . " OR (commentid like '%$key%' AND date_taken<='$value')";
 }
 $count++;
}
$w = $w . ")";
...
foreach ($instruments as $instrument) {
 ...
 $count = 0;
 foreach ($NAPFU24Date as $key => $value) { if (is_array($value)) continue;
  $nofail = " and c.PSCID = '$key' and i.Date_taken<='$value' ";
  $wherenofail = " where PSCID = '$key' ";
...
//                }
 ...
 if ($Test_name == 'genetics') {
  $queryAll = $queryAll . " order by PSCID";
 } else if ($Test_name == 'navigational_task_session_1' || $Test_name == 'navigational_task_session_2') {
  $queryAll = $queryAll;// . " order by candidate.PSCID";
 } else {
  $queryAll = $queryAll . " order by Visit_label, PSCID";
 }
	$DB->select($queryAll, $instrument_table);
...
$wherenofailnowhere = " AND candidate.PSCID in (select pscid from candidate where pscid in ('MTL0000', 'MTL0001', 'MTL0003', 'MTL0002', 'MTL0004', 'MTL0005'))";
/*
* Participant Status
*/
...
$noPOITrial = " AND td.Trial!='Probucol_Phase_1_Trial_A' ";
/*
* Treatment Duration
*/
$Test_name = "TreatmentDuration";
$query = "select PSCID, td.CandID, td.Trial, td.treatment_duration from treatment_duration td join candidate using (CandID) where PSCID not like '%MTL999%' and PSCID not like 'MTL0000' " . $wherenofailnowhere . $noPOITrial . " order by PSCID";
